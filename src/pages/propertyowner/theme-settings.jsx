...
import { 
  Sparkles, CheckCircle2, AlertCircle, Save 
} from "lucide-react";
    ...
    >
      <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4 mb-8">
        <div>
          ...
          <p className="mt-1.5 text-[13.5px] text-muted-foreground">Customize color palettes, accent highlights, and general dark/light modes options.</p>
        </div>
      </div>

      <div className="max-w-2xl bg-card border border-border rounded-2xl p-6 shadow-soft space-y-6">
        <h3 className="font-serif text-[20px] text-foreground border-b border-border/60 pb-3">Style Preferences</h3>
        ...
        {success && (
          <div className="rounded-xl border border-emerald-100 bg-emerald-50 p-4 text-emerald-700 text-xs font-bold flex items-center gap-2 animate-bounce">
            <CheckCircle2 size={16} /> Theme preferences saved successfully!
          </div>
        )}

        <form onSubmit={handleSave} className="space-y-6">
          <div>
            <label className="text-[10px] font-black text-slate-800 uppercase mb-3 block tracking-tight">Active Theme Layout</label>
            <select 
              value={theme} 
