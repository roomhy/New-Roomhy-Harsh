...
import { Search, MapPin, Star, BadgeCheck, TrendingUp, ChevronLeft, ChevronRight, ChevronDown, Building2, Home, Users } from 'lucide-react';
...
import { fetchCities, fetchProperties, trackPropertyClick, fetchJson } from './utils/api';
...
export default function HomePage() {
  useSEO({ pageKey: 'home', fallbackTitle: 'Roomhy - Premium Broker-Free Student & Professional Living' });
  ...
  const renderHeroSection = () => {
    ...
    const activeHeroImages = [
      heroContent.image1 || 'https://images.unsplash.com/photo-1502672260266-1c1ef2d93688?q=80&w=1980&auto=format&fit=crop',
      heroContent.image2 || 'https://images.unsplash.com/photo-1512917774080-9991f1c4c750?q=80&w=2070&auto=format&fit=crop',
      heroContent.image3 || 'https://images.unsplash.com/photo-1494203484021-3c454daf695d?q=80&w=2070&auto=format&fit=crop'
    ];
          ...
          <div className="relative max-w-none w-full mx-auto px-4 md:px-8 lg:px-12 h-full flex flex-col justify-start pt-4 md:pt-10">
            ...
            <div className="max-w-5xl mx-auto w-full px-2 md:px-4 search-container relative z-50">
              <form onSubmit={handleSearchSubmit} className="bg-white/95 md:bg-white backdrop-blur-md md:backdrop-blur-none rounded-2xl md:rounded shadow-2xl p-2 md:p-0 flex flex-row gap-2 md:gap-0 items-center relative z-20 md:h-16 w-full">
                ...
                <div className="hidden md:block relative border-r border-gray-200 h-full">
                  <div
                    onClick={() => setIsTypeDropdownOpen(!isTypeDropdownOpen)}
                    className="flex items-center justify-between bg-white text-gray-900 px-6 py-4 font-bold text-base cursor-pointer w-48 h-full"
                  >
                    <span>{selectedType ? offerings.find(o => o.category.toLowerCase() === selectedType)?.title : 'Property Type'}</span>
  ...
  const renderOfferingsSection = () => {
    const offeringContent = getSectionContent('offerings', {
      title: 'What We Offer',
      subtitle: 'Choose from a variety of accommodation types tailored for students'
    });
    const activeOfferings = offeringContent.list || offerings;
    return (
      <div key="offerings">
        ...
        <section className="py-1 md:py-4 bg-white">
          <div className="max-w-none w-full mx-auto px-4 md:px-8 lg:px-12 mt-1 md:mt-4">
            <div className="text-center mb-4">
              <h2 className="text-xl md:text-3xl font-bold text-gray-900 mb-1">{offeringContent.title}</h2>
              <p className="text-xs md:text-base text-gray-600">{offeringContent.subtitle}</p>
            </div>
            ...
            <div className="hidden md:grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 md:gap-6">
              {activeOfferings.map((offering) => {
                ...
                const allImages = offering.images || (offering.image ? [offering.image] : []);
                ...
                return (
                  ...
                  >
                    <div className="h-40 overflow-hidden relative">
                      <img
                        src={allImages[selectedIdx] || 'https://picsum.photos/600/400?random=2'}
                        alt={offering.title}
                  ...
                  {activeOfferings.map((offering) => (
                    ...
                    >
                      <div className="h-24 overflow-hidden relative group">
                        <img
                          src={offering.image || (offering.images && offering.images[0]) || 'https://picsum.photos/600/400?random=2'}
                          alt={offering.title}
  ...
  const renderPropertiesSection = () => {
    const propContent = getSectionContent('properties', {
        ...
        <section className="hidden md:block py-1 md:py-2 bg-white">
          <div className="max-w-none w-full mx-auto px-4 md:px-8 lg:px-12 mt-1 md:mt-2">
            <div className="flex flex-col items-center justify-center text-center mb-4">
              <h2 className="text-3xl font-bold text-gray-900 mb-1">{propContent.title}</h2>
              <p className="text-base text-gray-600">{propContent.subtitle}</p>
            </div>
                ...
                {visibleTrending.map((property) => (
                  ...
                  >
                    <div className="relative h-36 rounded-md overflow-hidden mb-2">
                      <img
                        src={property.image || `https://picsum.photos/600/400?random=${Math.floor(Math.random() * 100)}`}
                        alt={property.name}
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-1.5 mb-1 text-[11px]">
                        <div className="bg-[#1AB64F] text-white px-1 py-0.5 rounded text-[10px] font-bold flex items-center gap-0.5">
                          {property.rating || '4.5'} <Star className="w-2.5 h-2.5 fill-white text-white" />
                        </div>
                        <span className="text-gray-500">Excellent</span>
                      </div>
                      <div className="flex items-baseline gap-1.5">
                        <span className="text-base font-bold text-gray-900">
                          {property.monthlyRent ? `₹${property.monthlyRent.toLocaleString()}` : (property.price || '₹6,000')}
                        </span>
        ...
        <section className="md:hidden py-1 bg-gray-50">
          <div className="max-w-none w-full mx-auto px-4 md:px-8 lg:px-12">
            <div className="text-center mb-3">
              <h2 className="text-xl font-bold text-gray-900 mb-1">{propContent.title}</h2>
              <p className="text-xs text-gray-600">{propContent.subtitle}</p>
            </div>
                  ...
                  {trendingProperties.map((property) => (
                    ...
                    >
                      <div className="relative h-24 rounded-2xl overflow-hidden shadow-md mb-2">
                        <img
                          src={property.image || `https://picsum.photos/600/400?random=${Math.floor(Math.random() * 105)}`}
                          alt={property.name}
                          className="w-full h-full object-cover"
                          loading="lazy"
                          width="144"
                        ...
                        <div className="absolute bottom-2 left-2 bg-white/90 backdrop-blur rounded-md px-1.5 py-0.5 flex items-center gap-1 shadow-sm">
                          ...
                          <span className="text-[10px] font-bold text-gray-800">{property.rating || '4.5'}</span>
                        </div>
                      </div>
  ...
  const renderRecentlyViewedSection = () => {
    ...
    const recentContent = getSectionContent('recently-viewed', {
          ...
          <div className="max-w-none w-full mx-auto px-4 md:px-8 lg:px-12 mt-1 md:mt-2">
            <div className="text-center mb-2">
              <div className="flex items-center justify-center gap-2 mb-0.5">
                ...
                <h2 className="text-xl md:text-2xl font-bold text-gray-900">{recentContent.title}</h2>
              </div>
              <p className="text-[10px] md:text-sm text-gray-600">{recentContent.subtitle}</p>
            </div>
  ...
  const renderWhyChooseUsSection = () => {
    const whyChooseContent = getSectionContent('why-choose-us', {
      title: 'Why Choose Roomhy?',
      subtitle: "Built by students, for students. Here's why thousands trust us."
    });
    return <WhyRoomhy key="why-choose-us" title={whyChooseContent.title} subtitle={whyChooseContent.subtitle} list={whyChooseContent.list} />;
  };
  ...
  const renderTestimonialsSection = () => {
    ...
    const defaultReviews = [
      ...
      { name: "Vikram Singh", role: "Working Professional", rating: 5, text: "The ₹500 booking token is such a smart feature. It shows owners you're serious about renting.", avatar: "https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?w=100&h=100&fit=crop&crop=face" },
    ...
    const activeReviews = testimonialContent.list || testimonialContent.reviews || defaultReviews;
        ...
        <section className="py-2 md:py-4 bg-gradient-to-b from-white to-gray-50 overflow-hidden mt-2">
          <div className="max-w-none w-full mx-auto px-4 md:px-8 lg:px-12 mb-2 md:mb-4">
            <div className="text-center">
              <h2 className="text-xl md:text-3xl font-bold text-gray-900 mb-1">{testimonialContent.title}</h2>
              <p className="text-xs md:text-base text-gray-600">{testimonialContent.subtitle}</p>
            </div>
          </div>
          
          <div className="relative">
            <div className="flex animate-scroll-left hover:pause-animation">
              {[...Array(2)].flatMap((_, setIdx) => activeReviews.map((review, idx) => (
                <div key={`${setIdx}-${idx}`} className="flex-shrink-0 w-[280px] mx-2">
                  <div className="bg-white rounded-2xl p-4 shadow-lg border border-gray-100 h-full">
                    <div className="flex items-center gap-2 mb-3">
                      <img
                        src={review.avatar || 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=100&h=100&fit=crop&crop=face'}
                        alt={review.name}
                    ...
                    <div className="flex gap-1 mb-2">
                      {[...Array(5)].map((_, i) => (
                        <Star 
                          key={i}
                          className={`w-3 h-3 ${i < Number(review.rating || 5) ? 'fill-yellow-400 text-yellow-400' : 'text-gray-300'}`}
                        />
                      ))}
                    </div>
  ...
  const defaultOrder = ['hero', 'offerings', 'cities', 'properties', 'recently-viewed', 'why-choose-us', 'testimonials', 'faq'];
      ...
      <main className="min-h-screen">
        {activeOrder.map(sectionId => {
          ...
          switch (sectionId) {
            ...
            case 'offerings': return renderOfferingsSection();
            case 'cities': return null; // Rendered inside hero sub-nav
            case 'properties': return renderPropertiesSection();
            case 'recently-viewed': return renderRecentlyViewedSection();
            case 'why-choose-us': return renderWhyChooseUsSection();
