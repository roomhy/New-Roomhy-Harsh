lucide.createIcons();
        ...
        // Change API_URL to your backend URL - development use localhost:5001
        ...
        async function loadWebsite() {
            ...
            try {
                ...
                // Filter for online/offline based on current filter
                visits = visits.filter(v => (currentWebsiteFilter === 'online' && v.isLiveOnWebsite === true) || (currentWebsiteFilter === 'offline' && v.isLiveOnWebsite === false));
                ...
                tbody.innerHTML = visits.map(v => {
                    ...
                    const fieldPhotos = prop.photos || v.photos || [];
                    const allPhotos = [...profPhotos, ...fieldPhotos];
                    const visitId = v.visitId || v._id || '';
                    const propType = prop.propertyType || '';
                    const propArea = prop.area || '';
                    const propGender = prop.genderSuitability || '';
                    const propPhone = prop.ownerPhone || prop.contactPhone || '';
                    const propRent = prop.rent || v.monthlyRent || 0;

                    return `
                    <tr class="hover:bg-gray-50 transition-colors">
                        <td class="px-4 py-3 font-mono text-xs text-gray-600">${visitId ? visitId.slice(-8).toUpperCase() : '-'}</td>
                        ...
                        <td class="px-4 py-3">${propType || '-'}</td>
                        <td class="px-4 py-3">${propArea || '-'}</td>
                        <td class="px-4 py-3">${propGender || '-'}</td>
                        <td class="px-4 py-3">${prop.ownerName || '-'}</td>
                        <td class="px-4 py-3 text-sm">${propPhone || '-'}</td>
                        <td class="px-4 py-3 font-bold">₹${propRent}</td>
                        <td class="px-4 py-3">
        ...
        async function updateTabCounts() {
            try {
                const response = await fetch(`${API_URL}/api/approved-properties/all`);
        ...
        async function toggleWebStatus(propertyId) {
            try {
                const response = await fetch(`${API_URL}/api/approved-properties/${propertyId}/toggle-live`, {
                    method: 'PUT',
                    headers: { 'Content-Type': 'application/json' }
                });
        ...
        async function deleteProperty(propertyId) {
            ...
            try {
                const response = await fetch(`${API_URL}/api/approved-properties/${propertyId}`, {
                    method: 'DELETE',
                    headers: { 'Content-Type': 'application/json' }
                });
